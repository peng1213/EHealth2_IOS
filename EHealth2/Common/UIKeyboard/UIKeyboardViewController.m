...
- (id)firstResponder:(UIView *)navView {
	for (id aview in [self allSubviews:navView]) {
		if ([aview isKindOfClass:[UITextField class]] && [(UITextField *)aview isFirstResponder]&&![(UITextField *)aview isHidden]) {
			return (UITextField *)aview;
		}
		else if ([aview isKindOfClass:[UITextView class]] && [(UITextView *)aview isFirstResponder]&&![(UITextView *)aview isHidden]) {
...
-(void)toolbarButtonTap:(UIButton *)button {
	...
	for (id aview in [self allSubviews:objectView]) {
		if ([aview isKindOfClass:[UITextField class]] && ((UITextField*)aview).userInteractionEnabled && ((UITextField*)aview).enabled&&![(UITextView *)aview isHidden]) {
			[textFieldArray addObject:(UITextField *)aview];
		}
		else if ([aview isKindOfClass:[UITextView class]] && ((UITextView*)aview).userInteractionEnabled && ((UITextView*)aview).editable&&![(UITextView *)aview isHidden]) {
...
- (void)addToolbarToKeyboard {
	...
	for (id aview in [self allSubviews:objectView]) {
		if ([aview isKindOfClass:[UITextField class]]) {
            ...
            ((UITextField *)aview).clearButtonMode=UITextFieldViewModeWhileEditing;
		}
