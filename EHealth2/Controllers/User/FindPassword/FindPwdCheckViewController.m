//
//  FindPwdCheckViewController.m
//  EHealth2
//
//  Created by 刘祯 on 15/9/22.
...
@interface FindPwdCheckViewController ()
{
...
@implementation FindPwdCheckViewController
...
-(void)nextStepButtonClick
{
    ...
    if(isValid) {
        //采用ASIHttp框架来实现
        NSURL *url =[NSURL URLWithString:api_user_findpwdcheck];
...
-(void)requestFinished:(ASIHTTPRequest *)request{
    ...
    if(model.ResultCode==0 ){
        ...
        TUserInfo *reg= [[JsonTool defaultTool] getTUserInfoEntity:request.responseString withKey:@"model"];
        FindPwdViewController *secView =[[FindPwdViewController alloc] init];
        [secView setUser:reg];
